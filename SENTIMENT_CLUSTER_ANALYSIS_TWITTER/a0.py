...
def read_screen_names(filename):
    ...
    with open(filename) as fle:
        result_final = [j.strip('\n') for j in fle.readlines()]
    return result_final
...
def get_users(twitter, screen_names):
    ...
    for i in screen_names:
        b= {'screen_name':i}
        result = robust_request(twitter,'users/lookup',b)
        a.append(result.json()[0])
    return a
...
def get_friends(twitter, screen_name):
    ...
    """
    ###TODO
    c = {'screen_name' : screen_name, 'count' :5000}
    result = ([id for id in robust_request(twitter,'friends/ids',c)])
    #print(sorted(result))
...
def add_all_friends(twitter, users):
    ...
    """
    ###TODO
    for variabl in users:
        result2 = variabl['screen_name']
        variabl['friends'] = get_friends(twitter,result2)
    pass
...
def friend_overlap(users):
    ...
    a = len(users)
    for z in range(a):
        for y in range(z+1,a):
            vv1= users[z]
            vv2= users[y]    
            res = len(set(vv2['friends']) & set(vv1['friends']))
            olp.append((vv1['screen_name'],vv2['screen_name'],res))
    res1 = sorted(olp, key=lambda x:( x[0], x[1]))
    res2 = sorted(res1, key=lambda x:x[2],reverse = True)
    return res2
    pass
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    """
    ###TODO
#     d = ([screen_name for users['screen_name'] in robust_request(twitter,'users/lookup',c)])
#     return d
    
    Clinton_frnds = []
    Trump_frnds = []
    #Clinton_frnds = get_friends(twitter,'HillaryClinton')
    #Trump_frnds = get_friends(twitter,'realDonaldTrump')
    for i in users:
        if(i['screen_name']=="HillaryClinton"):
            Clinton_frnds = i['friends']
        elif(i['screen_name']=="realDonaldTrump"):
            Trump_frnds = i['friends']
    temp = set(Trump_frnds)
    med = temp.intersection(Clinton_frnds)
    Intersect_id = (list(med))[0]
        # Intersect_id = (set(Trump_frnds).intersection(Clinton_frnds))
    a = {'user_id':Intersect_id}
    res1 = robust_request(twitter,'users/lookup',a)
    result = res1.json()[0]['screen_name']
    return result
    pass
...
def create_graph(users, friend_counts):
    ...
    graph = nx.Graph()
    twofrnds = [i for i in friend_counts if friend_counts[i]>1]
    for cand in users:
        candy = cand['screen_name']
        graph.add_node(candy)
        for frnd in twofrnds:
            graph.add_node(frnd)
            if frnd in cand['friends']:
                graph.add_edge(frnd,cand['screen_name'])
    return graph
    pass
...
def draw_network(graph, users, filename):
    ...
    for nd in graph.nodes():
        if nd in users >= 4 :
            ...
            ''        
    namess=['DrJillStein', 'GovGaryJohnson', 'HillaryClinton', 'realDonaldTrump']
    for nd2 in graph.nodes():
        if nd2 in namess:
            labels[nd2] = nd2
            plt.figure(figsize=(20,12))
    nx.draw_networkx(graph,labels=labels,font_size=14,font_color='k', alpha=.5, width=.1,node_size=90)
    plt.axis("off")
